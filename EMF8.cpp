...
void DrawRuler8(HDC hdc, int cx, int cy)
{
	int iAdj, i, iHeight;
	LOGFONT lf;
	TCHAR ch;
...
void CreateRoutine8(HWND hwnd)
{
	...
	DrawRuler8(hdcEMF, 6 * xDpi, yDpi);
	hemf = CloseEnhMetaFile(hdcEMF);
	DeleteEnhMetaFile(hemf);
}
void PaintRoutine8(HWND hwnd, HDC hdc, int cxArea, int cyArea)
{
	ENHMETAHEADER emh;
	HENHMETAFILE hemf;
	int cxImage, cyImage;
