...
void DrawRuler12(HDC hdc, int cx, int cy)
{
	int iAdj, i, iHeight;
	LOGFONT lf;
	TCHAR ch;
...
void CreateRoutine12(HWND hwnd)
{
	HDC hdcEMF;
	HENHMETAFILE hemf;
	...
	DrawRuler12(hdcEMF, 6 * xDpi, yDpi);
	hemf = CloseEnhMetaFile(hdcEMF);
	DeleteEnhMetaFile(hemf);
}
void PaintRoutine12(HWND hwnd, HDC hdc, int cxArea, int cyArea)
{
	ENHMETAHEADER emh;
	HENHMETAFILE hemf;
	POINT pt;
